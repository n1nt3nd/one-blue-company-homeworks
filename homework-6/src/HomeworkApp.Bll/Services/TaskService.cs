...
public class TaskService : ITaskService
{
    ...
    public async Task<GetSubTaskModel[]> GetSubTasksInStatus(long parentTaskId, Bll.Enums.TaskStatus[] statuses, CancellationToken token)
    {
        var subTasksModels = await _taskRepository.GetSubTasksInStatus(
            parentTaskId,
            statuses.Select(x => x.ToDal()).ToArray(),
            token
        );

        var subTasks = subTasksModels
            .Select(x => x.ToBll())
            .ToArray();
