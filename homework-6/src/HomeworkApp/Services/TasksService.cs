...
public class TasksService : WorkshopApp.Proto.Client.TasksService.TasksServiceBase
{
    ...
    public override async Task<V1GetSubTasksResponse> V1GetSubTasks(V1GetSubTasksRequest request, ServerCallContext context)
    {
        var subTasks = await _taskService.GetSubTasksInStatus(
            request.ParentTaskId,
            request.Status.Select(x => x.ToBll()).ToArray(),
            context.CancellationToken
        );
        
        return new V1GetSubTasksResponse()
        {
            SubTasks =
            {
                subTasks.Select(x => new SubTask()
                {
                    TaskId = x.TaskId,
                    Title = x.Title,
                    Status = x.Status.ToGrpc(),
                    ParentTaskIds = {x.ParentTaskIds}
                }).ToArray()
            }
        };
    }
